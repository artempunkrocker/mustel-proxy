 ...
 * @internal
 */
type NumericIndex<T> = IsArray<T> extends true ? {
 ...
 * ```
 */
export type DeepPath<T, P extends unknown[]> = P extends [infer K, ...infer R] ? K extends keyof T ? DeepPath<T[K], R> : IsArray<T> extends true ? K extends number ? R extends [] ? DeepPath<ArrayElementType<T>, R> : DeepPath<ArrayElementType<T>, R> : never : never : T;
/**
 ...
 * ```
 */
type ProxyValue<V> = V extends (...args: infer A) => string ? (...args: A) => string : V extends Record<string, unknown> ? MustelProxy<V> : V;
/**
